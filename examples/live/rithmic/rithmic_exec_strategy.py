...
def _require_exec_state(key: str) -> _ExecState:
    ...
    if state is None:
        raise RuntimeError(f"Unknown Rithmic exec state key: {key}")
    return state
...
class RithmicExecStrategyConfig(StrategyConfig):
    def __new__(
        ...
        strategy_id: str | StrategyId = "RITHMIC-EXEC-001",
        state_key: str = "rithmic-exec-smoke",
        ...
        config.exec_client_id = _coerce_client_id(exec_client_id)
        config.account_id = _coerce_account_id(account_id)
        config.state_key = state_key
        config.entry_side = _coerce_order_side(entry_side)
        config.entry_qty = parsed_entry_qty
        config.entry_time_in_force = _coerce_time_in_force(entry_time_in_force)
        config.cleanup_on_start = bool(cleanup_on_start)
        config.cleanup_on_stop = bool(cleanup_on_stop)
        config.flatten_after_fill = bool(flatten_after_fill)
...
class RithmicExecStrategy(Strategy):
    def __init__(self, config: RithmicExecStrategyConfig):
    ...
    def on_start(self):
        ...
        self._info(
            f"Starting Rithmic execution strategy for {self.config.instrument_id} "
            f"via exec client {self.config.exec_client_id} snapshot={self._runtime_snapshot()}",
        )
    ...
    def on_order_filled(self, event):
        ...
        if self.config.flatten_after_fill and not self._flatten_requested:
            ...
            self.close_all_positions(
                instrument_id=self.config.instrument_id,
                client_id=self.config.exec_client_id,
    ...
    def _submit_entry_order(self):
        ...
        order = MarketOrder(
            ...
            instrument_id=self.config.instrument_id,
            client_order_id=ClientOrderId(f"RTHX-{uuid4().hex[:24].upper()}"),
            ...
            tags=["rithmic", "exec", "example"],
        )
        self._info(
            f"Submitting entry account={self.config.account_id} via={self.config.exec_client_id} "
    ...
    def _cancel_open_orders(self, stop_phase: bool = False):
        ...
        for order in orders:
            ...
            self.cancel_order(order, client_id=self.config.exec_client_id)
