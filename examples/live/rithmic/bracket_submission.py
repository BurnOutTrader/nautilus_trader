...
2. Loads a specific configured Rithmic contract
...
Example configuration:
    Edit the constants below for instrument ID, side, quantity, offsets,
    bracket distances, and hold time.
...
async def main() -> None:  # noqa: C901
    ...
    entry_side = parse_side(ENTRY_SIDE)
    entry_points_from_market = float(ENTRY_POINTS_FROM_MARKET)
    ...
    if entry_points_from_market <= 0:
        raise ValueError("ENTRY_POINTS_FROM_MARKET must be positive")

    if profit_ticks <= 0 or stop_ticks <= 0:
        raise ValueError("PROFIT_TICKS and STOP_TICKS must be positive")
    ...
    try:
        ...
        instrument = await provider.load_instrument_async(symbol, exchange)
        print(
