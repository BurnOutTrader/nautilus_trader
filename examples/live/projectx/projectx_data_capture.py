...
def register_capture_state(
    *,
    ...
    with _STATE_LOCK:
        _CAPTURE_STATES[key] = _CaptureState(
...
def _require_capture_state(key: str) -> _CaptureState:
    ...
    if state is None:
        raise RuntimeError(f"Unknown ProjectX capture state key: {key}")
    return state
...
class ProjectXDataCaptureStrategyConfig(StrategyConfig):
    def __new__(
        cls,
        instrument_id: str | InstrumentId = "MESM26.PROJECTX",
        client_id: str | ClientId = "PROJECTX",
        strategy_id: str | StrategyId = "PROJECTX-DATA-CAPTURE-001",
        state_key: str = "projectx-data-capture",
...
class ProjectXDataCaptureStrategy(Strategy):
    def __init__(self, config: ProjectXDataCaptureStrategyConfig):
