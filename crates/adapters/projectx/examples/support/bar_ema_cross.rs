...
#[derive(Debug, Clone)]
pub(crate) struct ProjectXBarEmaCrossConfig {
...
#[derive(Debug)]
pub(crate) struct ProjectXBarEmaCrossStrategy {
    core: StrategyCore,
    config: ProjectXBarEmaCrossConfig,
...
impl ProjectXBarEmaCrossStrategy {
    pub(crate) fn new(config: ProjectXBarEmaCrossConfig) -> Self {
    ...
    fn request_warmup_history(&mut self) -> anyhow::Result<()> {
        ...
        println!(
            "Requesting ProjectX warmup bars for {} over the last {} minutes",
    ...
    fn submit_signal(&mut self, side: OrderSide) -> anyhow::Result<()> {
        println!(
            "ProjectX EMA signal: side={side} instrument={} live_bars={} history_bars={}",
...
nautilus_strategy!(ProjectXBarEmaCrossStrategy, {
    fn external_order_claims(&self) -> Option<Vec<InstrumentId>> {
        Some(vec![self.config.instrument_id])
    }
});

impl DataActor for ProjectXBarEmaCrossStrategy {
    ...
    fn on_stop(&mut self) -> anyhow::Result<()> {
        ...
        println!(
            "Stopped ProjectX EMA strategy: live_bars={} history_bars={} warmup_complete={}",
    ...
    fn on_historical_bars(&mut self, bars: &[Bar]) -> anyhow::Result<()> {
        ...
        self.warmup_complete = true;
        println!(
            "ProjectX warmup complete: loaded {} historical bars for {}",
