...
async fn list_accounts_connected_path_authenticates_and_collects_multi_response_accounts() {
    ...
    let mut rx = gateway.subscribe_execution_events();
...
#[tokio::test]
async fn execution_event_subscriptions_fan_out_to_multiple_listeners() {
    let server = MockOrderPlant::start().await;
    let config = test_order_only_gateway_config(&server.url);
    let mut gateway = rithmic_nt::RithmicGateway::new(config);
    let mut rx_a = gateway.subscribe_execution_events();
    let mut rx_b = gateway.subscribe_execution_events();

    gateway.connect().await.unwrap();
    assert!(gateway.is_connected());

    expect_execution_authenticated(&mut rx_a).await;
    expect_execution_authenticated(&mut rx_b).await;
...
async fn request_pnl_snapshot_connected_path_emits_balance_and_position_updates() {
    ...
    let mut gateway = rithmic_nt::RithmicGateway::new(config);
    let mut rx = gateway.subscribe_pnl_events();
    ...
    loop {
            ...
            .expect("timed out waiting for execution event")
        {
            ...
            Err(tokio::sync::broadcast::error::RecvError::Closed) => {
                panic!("execution channel closed unexpectedly")
            }
        }
    }
}

async fn expect_execution_authenticated(rx: &mut tokio::sync::broadcast::Receiver<ExecutionEvent>) {
    for _ in 0..4 {
...
async fn next_pnl_event(rx: &mut tokio::sync::broadcast::Receiver<PnlEvent>) -> PnlEvent {
    loop {
        match tokio::time::timeout(Duration::from_secs(2), rx.recv())
            .await
            .expect("timed out waiting for pnl event")
        {
            ...
            Err(tokio::sync::broadcast::error::RecvError::Lagged(_)) => {}
            Err(tokio::sync::broadcast::error::RecvError::Closed) => {
                panic!("pnl channel closed unexpectedly")
            }
        }
    }
}
