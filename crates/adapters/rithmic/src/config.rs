...
pub fn configured_env_profiles() -> Result<Vec<String>> {
    match env::var(RITHMIC_PROFILES_ENV) {
        Ok(value) if !value.trim().is_empty() => parse_env_profiles_csv(&value),
        _ => Ok(Vec::new()),
    }
}
...
///
/// Returns the number of keys inserted into the process environment.
///
...
pub fn load_rithmic_env_file(path: Option<&str>) -> Result<usize> {
    ...
    for entry in dotenv_iter {
        ...
        if !key.starts_with("RITHMIC_") || env::var_os(&key).is_some() {
            continue;
        }
...
impl RithmicExecClientConfig {
    ...
    pub fn from_env_with_profile(profile: Option<&str>) -> Result<Self> {
        ...
        Ok(Self {
            trader_id: optional_env_var("TRADER_ID", profile)?
                .map_or_else(|| TraderId::from("TRADER-001"), TraderId::from),
            environment,
            username: required_env_var("USERNAME", profile)?,
            password: required_env_var("PASSWORD", profile)?,
            system_name: required_env_var("SYSTEM_NAME", profile)?,
...
mod tests {
    ...
    #[rstest::rstest]
    fn test_load_rithmic_env_file_sets_only_missing_rithmic_keys() {
        let _guard = ENV_LOCK.lock().unwrap();
        let previous = [
            (
                "RITHMIC_USERNAME",
        ...
        assert_eq!(std::env::var("RITHMIC_APP_NAME").as_deref(), Ok("OwnApp"));
