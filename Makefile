...
ADAPTER_CRATES := nautilus-architect-ax nautilus-betfair nautilus-binance \
    ...
    projectx-nt rithmic-nt nautilus-sandbox nautilus-tardis
...
.PHONY: cargo-test-crate-projectx-nt-all-targets
cargo-test-crate-projectx-nt-all-targets: export RUST_BACKTRACE=1
cargo-test-crate-projectx-nt-all-targets: check-nextest-installed
cargo-test-crate-projectx-nt-all-targets:  #-- Run ProjectX library and integration tests
	cargo nextest run --lib --tests $(FAIL_FAST_FLAG) --profile $(NEXTEST_PROFILE) --cargo-profile $(CARGO_CI_PROFILE) -p projectx-nt --features "$$(./scripts/crate-test-features.sh projectx-nt)" $(NEXTEST_OUTPUT_ARGS)
